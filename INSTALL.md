...
The *PushButton* library can be installed in two different ways: cloning this repository using Git on your host, or downloading the repository as a ZIP file and installing it from the Arduino IDE.  For either procedure you need to know where your IDE stores user-installed libraries.
...
1. From a web browser, go to this library's repository at [https://github.com/twrackers/PushButton-library](https://github.com/twrackers/PushButton-library).
...
1. In your command shell, type "git clone ", then paste the URL from the previous step into your command line.  You should see:
	...
	`$ git clone https://github.com/twrackers/PushButton-library.git`
1. Hit `Enter` to clone the repository into your `libraries` directory.
2. If your Arduino IDE is running, close it now.
3. If you type `ls -l`, you will see a list of your libraries, including one named `PushButton-library-main`.  It needs to be renamed:
 
	`$ mv PushButton-library-main PushButton`
1. You can now start the Arduino IDE again.  If from the menu you click *Sketch* -> *Include Library*, you should be able to scroll down to find `PushButton`.
...
7. Navigate to your `libraries` directory.  You will see a directory named `PushButton-library-main`.  Rename it to simply `PushButton`.
8. You can now start the Arduino IDE again.  If from the menu you click *Sketch* -> *Include Library*, you should be able to scroll down to find `PushButton`.